console.log("--- SCRIPT.JS PARSE START ---");
...
window.onload = () => {
    ...
    try {
        ...
        function setGameState(newState) {
            ...
            const showHud = (newState === 'playing' || newState === 'paused' || newState === 'gameOver');
            ui.hud.classList.toggle('hidden', !showHud);
            ui.dashButtonMobile.classList.toggle('hidden', !isMobile || !showHud);


            // Hide all panels with a fade-out animation
            for (const panelKey in ui) {
                const panel = ui[panelKey];
                if (panel && panel.classList.contains('ui-panel') && !panel.classList.contains('hidden')) {
            ...
            if (newState === 'menu') {
                panelToShow = ui.mainMenu;
                updateGemDisplay();
            } else if (newState === 'paused') {
                panelToShow = ui.pauseMenu;
                ...
                panelToShow = ui.levelUpScreen;
            } else if (newState === 'guide') {
                panelToShow = ui.guideScreen;
            } else if (newState === 'rank') {
                showRank();
                panelToShow = ui.rankScreen;
            } else if (newState === 'upgrades') {
                populateUpgradesMenu();
                panelToShow = ui.upgradesMenu;
            } else if (newState === 'characterSelect') {
                populateCharacterSelectScreen();
                panelToShow = ui.characterSelectScreen;
            } else if (newState === 'achievements') {
                populateAchievementsScreen();
                panelToShow = ui.achievementsScreen;
            } else if (newState === 'loading') {
                panelToShow = ui.loadingScreen;
            }

            if (panelToShow) {
                // Use a timeout to show the new panel after the old one has started fading
                setTimeout(() => {
                    panelToShow.classList.remove('hidden');
                    // Another tiny timeout to allow the browser to apply display:block before starting the transition
                    setTimeout(() => {
